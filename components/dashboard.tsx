...
export function Dashboard() {
  ...
  const timelineData = useMemo(() => {
    ...
    // 本来は日付ごとに集計しますが、デモ用に直近のデータを並べます
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl gap-1">
            {(['1W', '1M', '1Y'] as const).map((r) => (
              <Button
                ...
                onClick={() => setRange(r)}
                className={cn(
                  "rounded-xl px-4 py-1 h-8 text-xs font-bold transition-all",
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={timelineData}>
              ...
              <Line 
                ...
                activeDot={{ r: 8, shadow: "0 0 10px rgba(99,102,241,0.5)" }}
              />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-6">
          <Card className="bg-white border-none shadow-sm rounded-[40px] p-10 text-center flex-1 flex flex-col justify-center transition-transform hover:scale-[1.02]">
            ...
            <h3 className="text-5xl font-black italic text-rose-500 tracking-tighter">¥{totalSpent.toLocaleString()}</h3>
          </Card>
          <Card className="bg-white border-none shadow-sm rounded-[40px] p-10 text-center flex-1 flex flex-col justify-center transition-transform hover:scale-[1.02]">
            <p className="text-[15px] font-bold uppercase tracking-[0.3em] text-indigo-400 mb-4">今日の総 GIFT</p>
            <h3 className="text-5xl font-black italic text-indigo-500 tracking-tighter">¥{totalReceived.toLocaleString()}</h3>
          </Card>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-10">
          {personStats.map((stat) => (
            <div key={stat.name} className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 space-y-8 transition-all hover:shadow-md">
              <div className="flex flex-col lg:flex-row justify-between items-start gap-8 leading-none">
                ...
                <div className="flex flex-col lg:flex-row lg:flex-1 w-full gap-8 lg:gap-12">
                  ...
                  <div className="w-full pt-4 border-t border-slate-50 space-y-3 order-2 lg:hidden">
                    <span className="text-[14px] font-bold text-slate-400 tracking-[0.2em] uppercase ml-1 block">感情のバランス</span>
              ...
              {/* PC版用感情バー */}
              <div className="hidden lg:block pt-8 border-t border-slate-50 space-y-4">
